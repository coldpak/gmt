...
- **최근 프로젝트** — `cd` 할 때마다 git 프로젝트 자동 추적, `gm go 1`로 바로 이동
- **할 일 관리** — `gm add 할 일 내용`으로 간단한 태스크 관리 (따옴표 없이도 OK)
- **오늘의 목표** — `gm goal 목표`로 설정 (자정에 자동 만료)
...
|--------|------|
| `gm` | 홈 화면 보기 |
| `gm add 할 일 내용` | 할 일 추가 (따옴표 없이도 OK) |
| `gm done <N>` | 완료/미완료 토글 |
| `gm rm <N>` | 할 일 삭제 |
| `gm goal 목표` | 오늘의 목표 설정 (따옴표 없이도 OK) |
| `gm go <N>` | 최근 프로젝트로 이동 |
| `gm list` | 할 일 목록만 표시 |
| `gm clear` | 완료된 항목 일괄 삭제 |
| `gm config` | 설정 파일 편집 |
| `gm setup` | 온보딩 다시 실행 |
| `gm help` | 도움말 |
| `gm version` | 버전 표시 |
...
`~/.gmt/config.sh`를 직접 편집하거나 `gm config` 명령어를 사용하세요.
...
모든 사용자 노출 문자열은 `L_` 변수로 관리됩니다. 명령어(`gm add` 등)는 번역하지 않습니다.
...
- **버그 리포트** — OS, 쉘 버전, `gm version` 출력과 함께 이슈를 열어주세요
